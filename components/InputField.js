import { Cat } from "lucide-react-native";
import { useState } from "react";
import {
  Button,
  Dimensions,
  StyleSheet,
  TextInput,
  TouchableOpacity,
  View,
} from "react-native";
import { ChatHistoryProvider, useChatHistory } from "../hooks/useChatHistory";
import { useTheme } from "../hooks/themeContext";

const { width } = Dimensions.get("window");

export function InputField() {
  const [chatContent, setChatContent] = useState("");
  const { sendMessage } = useChatHistory();
  const { palette } = useTheme();
  const chatPlaceHolder = "Type to chat..";

  const textStyle = { color: palette.textOnSurface };
  const surfaceStyle = { backgroundColor: palette.surface };
  const placeholderStyle = { color: palette.placeholder };
  const surfaceAltStyle = { backgroundColor: palette.surfaceAlt };

  const handleSend = async () => {
    if (!chatContent.trim()) {
      return;
    }
    const chat = chatContent;
    setChatContent("");
    try {
      await sendMessage(chat);
    } catch (e) {
      console.error("something went wrong sending to the bot");
    }
  };

  return (
    <View style={[style.InputContainer, surfaceStyle]}>
      <TextInput
        multiline
        style={[style.inputfield, surfaceStyle, palette.textOnSurface]}
        value={chatContent}
        onChangeText={setChatContent}
        placeholder={chatPlaceHolder}
      ></TextInput>

      <TouchableOpacity
        style={[style.sendButton, surfaceAltStyle]}
        onPress={handleSend}
      >
        <Cat size={25} color={"pink"} />
      </TouchableOpacity>
    </View>
  );
}

const style = StyleSheet.create({
  InputContainer: {
    flexDirection: "row",
    alignItems: "center",
    position: "absolute",
    elevation: 5,
    width: width - 10,
    maxHeight: 120,
    minHeight: "fit-content",
    bottom: 5,
    borderRadius: 24,
    overflow: "hidden",
    margin: 5,
  },

  inputfield: {
    flex: 1,
    width: width - 60,
    fontSize: 15,
    paddingLeft: 4,
    marginLeft: 5,
  },

  sendButton: {
    justifyContent: "center",
    alignItems: "center",
    width: 45,
    height: 45,
    borderRadius: 24,
    margin: 5,
    marginRight: 5,
  },
});
